...
namespace Alpaca
{
	...
	template<typename T>
	class RequestBody : public JSONInfoBody
	{
			DEVICEPROP(T, Value, value)
			DEVICEPROP(unsigned int, ClientID, clientID)
	...
	public:
		...
		{
			m_value               = jsonUtils::extractValue<T>(info, "Value", "");
			m_clientTransactionID = jsonUtils::extractValue<unsigned int>(info, "ClientTransactionID", 0);
			m_clientID            = jsonUtils::extractValue<unsigned int>(info, "ClientID", 0);
		}
	};
	typedef RequestBody<bool>          BoolRequestBody;
	typedef RequestBody<int>           IntRequestBody;
	typedef RequestBody<std::string>   StringRequestBody;
	typedef RequestBody<double>		   DoubleRequestBody;
